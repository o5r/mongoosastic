...
function Mongoosastic (schema, pluginOpts) {
  ...
  function postSave (doc) {
    ...
    function onIndex (err, res) {
      if (!filter || !filter(_doc)) {
        _doc.emit('es-indexed', err, res)
      } else {
        _doc.emit('es-filtered', err, res)
      }
    }

    if (doc) {
      if (this.options && this.options.rawResult) {
        _doc = new doc.constructor(doc.value)
      } else {
